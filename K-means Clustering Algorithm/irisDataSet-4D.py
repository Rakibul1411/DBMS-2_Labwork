...
def plot_clusters(X, clusters, centroids):
    ...
    # 4D visualization (using color and size for additional dimensions)
    ax3 = fig.add_subplot(133)
    for i in range(len(centroids)):
        cluster_points = X[clusters == i]
        ax3.scatter(cluster_points[:, 0], cluster_points[:, 1], c=cluster_points[:, 2], s=cluster_points[:, 3] * 20, label=f"Cluster {i+1} (Size: {len(cluster_points)})", alpha=0.6, cmap='viridis')
    ax3.scatter(centroids[:, 0], centroids[:, 1], c='red', marker='x', s=200, label='Centroids')
    ax3.set_title("4D View (Color: Feature 3, Size: Feature 4)")
